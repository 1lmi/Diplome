declare global {
  interface Window {
    ymaps?: any;
    __scRestaurantYandexMapsPromise?: Promise<any>;
  }
}
...
export function loadYandexMaps(): Promise<any> {
  ...
  if (window.__scRestaurantYandexMapsPromise) {
    return window.__scRestaurantYandexMapsPromise;
  }

  window.__scRestaurantYandexMapsPromise = new Promise((resolve, reject) => {
